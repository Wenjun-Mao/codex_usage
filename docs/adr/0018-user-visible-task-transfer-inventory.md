...
Persisted index project provenance remains authoritative when discovery relocates a missing indexed task. Selected bytes must validate against that persisted key and its aliases before the relocated entry becomes repair-ready. A declared repository identity has primary authority and can match only the indexed canonical repository or a repository alias; a matching path alias cannot override a conflicting repository. Path-only tasks may still match declared path aliases. Metadata-only reconstructed entries remain browse-visible but are not commit-ready. Index commits preserve persisted catalog entries, overlay the latest persisted state, and then add only complete repairs and explicit task changes, so unrelated unselected discoveries are neither hashed nor accidentally persisted.
