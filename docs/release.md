# 2.8.1 Distribution Checklist

Version 2.8.1 publishes standalone macOS Apple Silicon and Windows x64 VSIX
...
Confirm all Python, npm, Cargo, Tauri, and lockfile versions are `2.8.1`, both
changelogs have a dated `2.8.1` entry, and the candidate commit is contained in
`origin/main`.

The only valid release tag for this version is `v2.8.1`. Create and push that
...
```text
Codex-Usage-2.8.1-macos-arm64-unsigned-preview.dmg
Codex-Usage-2.8.1-windows-x64-unsigned-preview-setup.exe
