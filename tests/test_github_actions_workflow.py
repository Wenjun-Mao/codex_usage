...
def test_release_metadata_is_stable_1_7_7():
    ...
    assert pyproject["project"]["version"] == "1.7.7"
    assert __version__ == "1.7.7"
    assert codex_usage_lock["version"] == "1.7.7"
    assert extension_package["version"] == "1.7.7"
    assert "preview" not in extension_package
    assert extension_lock["version"] == "1.7.7"
    assert extension_lock["packages"][""]["version"] == "1.7.7"
