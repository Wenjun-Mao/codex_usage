...
def test_release_document_uses_current_tag_example() -> None:
    ...
    assert "such as `v1.5.0`" in release_document
...
def test_release_metadata_is_stable_1_5_0():
    ...
    assert codex_usage_lock["version"] == "1.5.0"
    assert extension_package["version"] == "1.5.0"
    assert "preview" not in extension_package
    assert extension_lock["version"] == "1.5.0"
    assert extension_lock["packages"][""]["version"] == "1.5.0"
