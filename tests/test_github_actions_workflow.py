...
def test_native_build_scripts_run_packaged_sync_smoke(build_script: Path):
    ...
    assert "packaged_task_backup_smoke.py" in text
...
def test_release_document_uses_current_tag_example() -> None:
    ...
    assert "such as `v1.6.1`" in release_document
...
def test_release_metadata_is_stable_1_6_1():
    ...
    assert codex_usage_lock["version"] == "1.6.1"
    assert extension_package["version"] == "1.6.1"
    assert "preview" not in extension_package
    assert extension_lock["version"] == "1.6.1"
    assert extension_lock["packages"][""]["version"] == "1.6.1"
