...
test("bundled collector selection is limited to released VSIX targets", () => {
  ...
  assert.equal(
    bundledAgentRelativePath("darwin", "arm64"),
    path.join("bin", "darwin-arm64", "codex-usage-agent"),
  );
  assert.equal(
    bundledAgentRelativePath("win32", "x64"),
    path.join("bin", "win32-x64", "codex-usage-agent.exe"),
  );
  assert.throws(() => supportedAgentTarget("linux", "x64"), /macOS Apple Silicon and Windows x64/);
  assert.throws(() => supportedAgentTarget("darwin", "x64"), /macOS Apple Silicon and Windows x64/);
  assert.throws(() => supportedAgentTarget("win32", "arm64"), /macOS Apple Silicon and Windows x64/);
});
